...
public class FreestyleTest {
    ...
    public void freestyle_conversion() throws Exception {
        ...
        {
            BuildTimeoutWrapper buildTimeoutWrapper = new BuildTimeoutWrapper(
...

        {
            ...
            parametersDefinitions.add(
                    new ChoiceParameterDefinition("choice", new String[] {"choice1", "choice2"}, "description choice"));
    ...
    public void freestyle_conversion_then_run() throws Exception {
        ...
        {
            BuildTimeoutWrapper buildTimeoutWrapper = new BuildTimeoutWrapper(
                    new NoActivityTimeOutStrategy("180"), Collections.singletonList(new FailOperation()), "FOO");
            p.getBuildWrappersList().add(buildTimeoutWrapper);
        }
        ...
        //        testFile.copyFrom( Thread.currentThread().getContextClassLoader().getResourceAsStream(
        // "junit-report.xml" ) );
    ...
    public void freestyle_conversion_only_Jenkinsfile() throws Exception {
        ...
        // parametersDefinitions.add( new ChoiceParameterDefinition( "choice", new String[]{"choice1","choice2"},
        // "description choice" ) );
        parametersDefinitions.add(new BooleanParameterDefinition("nameboolean", true, "boolean description"));
        ParametersDefinitionProperty parametersDefinitionProperty =
                new ParametersDefinitionProperty(parametersDefinitions);
    ...
    public void freestyle_conversion_maven_build_then_run_project() throws Exception {
...

        {
            ...
            GlobalMavenSettingsConfig globalMavenSettingsConfig = new GlobalMavenSettingsConfig(
                    "global-maven-settings-id", "global-maven-settings-name", "comment", content);
            ...
            Maven maven = new Maven(
                    "clean verify",
                    mavenInstallation.getName() /*maven name*/,
                    "pom.xml",
                    "-DskipTests" /*properties*/,
                    "-Djava.awt.headless=true" /*jvmOptions*/,
                    false /*usePrivateRepository*/,
                    null /*SettingsProvider settings*/,
    ...
    @Test
    public void get_warning() throws Exception {
        Slave slave = j.createSlave("FOO_AGENT", null);
        ...
        converter.convert(request, converterResult, p);
        assertEquals(
                converterResult.getWarnings().toString(),
                5,
                converterResult.getWarnings().size());
        ...
        assertThat(
                groovy,
                containsString("No converter for Builder: io.jenkins.plugins.todeclarative.converter.FakeBuilder"));
        assertThat(
                groovy,
                containsString("No converter for Publisher: io.jenkins.plugins.todeclarative.converter.FakeRecorder"));
