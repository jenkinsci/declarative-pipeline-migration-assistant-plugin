...
public class FreestyleToDeclarativeConverter extends SingleTypedConverter<FreeStyleProject> {
    ...
    public boolean convert(ConverterRequest request, ConverterResult result, Object target) throws ConverterException {
...

        {
            ...
            if (!freeStyleProject.isConcurrentBuild()) {
                result.addWarning(new Warning(
                        "Current Freestyle project is marked as no concurrent build but it is default option for pipeline",
                        null));
            }
        }
        ...
        convertTypeList(
                request, result, converterList, freeStyleProject.getTriggers().values());
        Map<JobPropertyDescriptor, JobProperty<? super FreeStyleProject>> projectProperties =
                freeStyleProject.getProperties();
        // ignore JiraProjectProperty as it is not removed when removing jira publishers so we avoid false positive
        projectProperties
                .entrySet()
                .removeIf(entry -> entry.getKey().clazz.getName().equals("hudson.plugins.jira.JiraProjectProperty"));
